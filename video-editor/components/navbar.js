            ...
            >
                <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
                    ...
                    <div className="md:hidden flex items-center justify-between">
                        {/* Logo/Brand Space (if needed) */}
                        <div className="w-8"></div>

                        {/* Center navigation for mobile */}
                        <div className="flex items-center space-x-3">
                            {/* Main nav items - condensed for mobile */}
                            <div className="backdrop-blur-md bg-black/30 border border-white/10 rounded-full px-4 py-2.5 shadow-lg">
                                <div className="flex items-center space-x-4">
                                    {navItems.slice(0, 2).map((item) => (
                                        <button
                                            key={item.name}
                                            onClick={() => handleNavClick(item.id)}
                                            className="text-white/60 hover:text-white font-normal text-sm transition-all duration-300 ease-out"
                                        >
                                            {item.name}
                                        </button>
                                    ))}
                                ...
                                <div className="p-6 space-y-1">
                                    ...
                                    >
                                        <a
                                            ...
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="block w-full text-center px-4 py-3 bg-white/10 hover:bg-white/20 text-white rounded-lg font-medium text-base transition-all duration-200"
                                            onClick={() => setIsMobileMenuOpen(false)}
                                        >
                                            Book a Call
                                        </a>
