...
export default function VideosPortfolio() {
  ...
  // Video Card with scroll animation
  const VideoCard = ({ video, isShort = false }) => (
    ...
    >
      <div
        className={`relative overflow-hidden rounded-lg sm:rounded-xl bg-gray-900 shadow-xl sm:shadow-2xl transition-all duration-500 ${
          isShort
        ...
        <div ref={longFormRef} className="mb-16 sm:mb-20 lg:mb-24">
          ...
          <div className="flex justify-center">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-6xl w-full">
              {longFormVideos.slice(3).map(video => (
                <VideoCard key={video.id} video={video} />
              ))}
            </div>
          </div>
